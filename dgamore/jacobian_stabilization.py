...
class JacobianTracker:
    ...
    def spectrum_state(self, shape: tuple[int, ...], beta: float, converged: bool, dtype: np.dtype) -> dict:
        ...
        :func:`predict_rung_crossing`, and re-derives the damping bound from the carried spectrum itself.
    ...
    def carry_in(
        ...
        point (either case carries the set as without the extrapolation), and both papers recommend exactly this
        preemptive flip for a carried Jacobian (arXiv:2606.04936 flips the modes with a very small positive real
        part or approaching a pole, the supplement of Phys. Rev. Lett. doi:10.1103/zjy7-4jqd Sec. VI likewise). A
    ...
    def update(self, iterates: list[np.ndarray], proposals: list[np.ndarray], allow_flip: bool = True) -> bool:
        ...
        *consecutive* updates, which gates the reversed sign of the per-direction map exactly as it gates the
        ...
        left out of the map and keeps the step the mixing gave it. The map itself is built from the current
        estimate on every update that certifies a mode, whether or not a reflector is installed, and an update that
        ...
        if allow_flip and may_install and gated.any():
            ...
            refreshed = _nonuniform_map(q_kept, form, theta, self.last_lam_pi, signs, mapped, self._log_info)
            if self.q is None:
