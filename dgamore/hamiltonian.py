...
:math:`t_{12}(R)`, the local interaction :math:`U_{1234}` and the non-local interaction :math:`V_{1234}(R)`. It offers
convenience builders (single-band, orbital-diagonal, Kanamori, 2D :math:`t`-:math:`t'`-:math:`t''`), readers/writers for
wannier90 / wien2k ``hr``/``hk`` and ``umatrix`` files, and Fourier transforms to obtain the band dispersion
:math:`\varepsilon_{12}(\mathbf{k})` and momentum-dependent interaction :math:`V_{1234}(\mathbf{q})`.
:class:`HoppingElement` and :class:`InteractionElement` are small validated containers for single hopping/interaction
entries.
"""
...
class Hamiltonian:
    """
    The Hamiltonian of the Hubbard model: the hopping terms (``er``) plus the local (``local_interaction``) and
    nonlocal (``nonlocal_interaction``) interaction terms, with a handful of builders that make adding terms
    convenient.
    """
    ...
    def kanamori_interaction_p(self, n_bands: int, upp: float, jpp: float, vpp: float = None) -> "Hamiltonian":
        ...
        :param upp: The intra-orbital Hubbard interaction :math:`U_{\mathrm{pp}}`.
        :param jpp: The Hund's exchange :math:`J_{\mathrm{pp}}`.
        :param vpp: The inter-orbital interaction :math:`V_{\mathrm{pp}}`; defaults to :math:`U_{\mathrm{pp}} - 2
            J_{\mathrm{pp}}` if None.
        :return: ``self`` (for chaining).
        """
    ...
    def read_hk_w2k(self, fname: str, spin_sym: bool = True):
        r"""
        Reads a Hamiltonian :math:`H_{bb'}(\mathbf{k})` from a text file.
        ...
        :return: A pair ``(ham, kpoints)`` where ``ham`` is a new :class:`Hamiltonian` with ``ek`` set to the complex
            array :math:`H(\mathbf{k})` (k-point first, then band indices) and ``kpoints`` is the real array of k-point
            ``(x, y, z)`` components (transposed).
    ...
    def get_ek(self, k_grid: bz.KGrid = None) -> np.ndarray:
        r"""
        Returns the band dispersion :math:`\varepsilon_{12}(\mathbf{k})`, computing and caching it from the real-space
        hopping on ``k_grid`` if not already set.
    ...
    def get_vq(self, q_grid: bz.KGrid) -> "Interaction":
        r"""
        Returns the momentum-dependent non-local interaction :math:`V_{1234}(\mathbf{q})` by Fourier-transforming the
    ...
    def _convham_4_orbs(self, k_mesh: np.ndarray) -> np.ndarray:
        r"""
        Fourier-transforms the real-space non-local interaction to :math:`V_{1234}(\mathbf{q})`, looping over lattice
        vectors to keep the memory footprint low.
