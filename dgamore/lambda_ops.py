...
class LambdaCorrection:
    ...
    def find_lambda_bounded(
        ...
        left open by an everywhere-positive susceptibility is moved outward by doubling its distance from the
        anchoring bound until the residual turns negative, giving up after ``_MAX_BRACKET_DOUBLINGS`` doublings, in
        which case the no-root fallback below is taken. A closed upper end whose residual is not negative - the
        imaginary part of the susceptibility softens the pole there into a feature that can be wider than the branch
        - is replaced by the first of ``64`` equal steps across the branch that is. The residual is evaluated in
        double precision, in which the near-pole ends are resolved.

        :param chi_r_mat: Physical susceptibility in the irreducible BZ, shape ``[q, w]``.
        :param chi_r_loc_sum: Target value: the local susceptibility sum (already divided by :math:`\beta`).
        :param lambda_previous: The previous iteration's :math:`\lambda`, a genuine sum-rule root, which halves the
            bracket when it lies inside the branch.
        ...
        :return: A tuple of (i) the :math:`\lambda` solving the sum rule inside the branch, or, when the residual
            has no sign change there, the no-root fallback, and (ii) whether that fallback was taken. The fallback
            is ``lambda_previous`` when it lies inside the bracket, else the neutral value: zero when zero lies
            inside the bracket, the branch midpoint on a closed branch, or the anchoring bound plus ``delta`` on an
            open one.
        """
        ...
        if np.isinf(upper):
            ...
            for _ in range(LambdaCorrection._MAX_BRACKET_DOUBLINGS):
                ...
                # no sign change within reach of the doublings: the no-root fallback below decides
        ...
        if upper <= lower or residual(lower) <= 0.0:
            has_genuine_previous = lambda_previous is not None and branch_lower < lambda_previous < branch_upper
            if has_genuine_previous:
                fallback, which = lambda_previous, f"the previous iteration's lambda {lambda_previous:.6f}"
            elif branch_lower < 0.0 < branch_upper:
                fallback, which = 0.0, "zero (no correction)"
            elif np.isinf(branch_upper):
                fallback, which = lambda_lo + delta, "the lower end plus delta"
            else:
                fallback, which = 0.5 * (branch_lower + branch_upper), "the branch midpoint"
        ...
        return float(opt.brentq(residual, lower, upper, xtol=eps)), False
    ...
    def perform_single(
        ...
        :param lambda_previous: The per-iteration :math:`\lambda` of each corrected channel, keyed by the channel
            value: when given, :meth:`find_lambda_bounded` runs warm-started with this channel's entry and the
            determined :math:`\lambda` is written back into the dict only when it is a genuine sum-rule root, so a
            fallback value never persists into the next iteration's warm start; ``None`` runs :meth:`find_lambda`.
        ...
        if lambda_previous is None:
            ...
            lambda_r, is_fallback = LambdaCorrection.find_lambda_bounded(
    ...
    def perform(chi_phys_q_r: FourPoint, quiet: bool = False, lambda_previous: dict | None = None) -> FourPoint:
        ...
        :param lambda_previous: The per-iteration :math:`\lambda` of each corrected channel, keyed by the channel
            ...
            from this dict and written back into it only when it is a genuine sum-rule root; ``None`` uses the
            Newton search of :meth:`find_lambda`.
