...
_JACOBIAN_ROW_WIDTH = 3  # leading Ritz values per row of the per-iteration eigenvalue files
...
def get_starting_sigma(default_sigma: SelfEnergy) -> tuple[SelfEnergy, int]:
    ...
    files = _iteration_files(previous_sc_path, "sigma_dga_iteration_*.npy")
    if not files:
        return default_sigma, 0
    max_iter = max(files)
    ...
    return _load_saved_sigma(max_file), max_iter


def _iteration_files(path: str, glob_pattern: str) -> dict[int, str]:
    """
    Maps the iteration number to the file for every per-iteration file of a previous run matching a pattern
    (file names ending in ``iteration_<i>.npy``). The files are looked up in the run's ``Sigma_Iterates``
    subfolder and, when that holds none, in the run folder itself.

    :param path: Folder of the previous self-consistency run.
    :param glob_pattern: File-name glob of the wanted per-iteration files.
    :return: Dictionary ``{iteration: file path}``.
    """
    files = glob.glob(os.path.join(path, config.output.sigma_iterates_subfolder_name, glob_pattern)) or glob.glob(
        os.path.join(path, glob_pattern)
    )
    return {int(match.group(1)): f for f in files if (match := re.search(r"iteration_(\d+)\.npy$", f))}


def _load_saved_sigma(path: str) -> SelfEnergy:
    """
    Loads a saved self-energy, cut to the core box and interpolated onto the current momentum grid.

    :param path: Path of the saved ``.npy`` self-energy.
    :return: The :class:`SelfEnergy`.
    """
    mat = np.load(path)
    sigma = SelfEnergy(mat, mat.shape[:3], True, False, beta=config.sys.beta).cut_niv(config.box.niv_core)
    return sigma.interpolate_q_grid(config.lattice.k_grid.nk, False)
...
def _jacobian_expander(state: dict) -> Callable[[np.ndarray], np.ndarray] | None:
    ...
    def expand(column: np.ndarray) -> np.ndarray:
        ...
        return to_vec(sigma.mat * inside)
...
def _carry_jacobian_spectrum(tracker: JacobianTracker, annealer: LambdaAnnealer | None) -> None:
    ...
    read from ``jacobian_spectrum.npz`` in ``previous_sc_path`` (see :meth:`JacobianTracker.save_spectrum`). A
    predecessor that did not reach the pure fixed point is carried with a warning; a predecessor the expander
    refuses (see :func:`_jacobian_expander`) is not carried. Flips are kept pending while a susceptibility-reshaping
    scaffold is on, as in :func:`_update_jacobian_tracker`.
    ...
    :return: None.
    """
    path = os.path.join(config.self_consistency.previous_sc_path, SPECTRUM_FILE)
    if not os.path.isfile(path):
        config.logger.info(f"No Jacobian spectrum file in {config.self_consistency.previous_sc_path}; nothing carried.")
        return
    ...
    tracker.carry_in(state, expand, allow_flip=allow_flip)


def _save_jacobian_spectrum(tracker: JacobianTracker, converged: bool) -> None:
    """
    Writes the tracker's certified spectrum to ``jacobian_spectrum.npz`` in the output folder for a successor run
    (see :meth:`JacobianTracker.save_spectrum`), on the full momentum grid.
    ...
    tracker.save_spectrum(os.path.join(config.output.output_path, SPECTRUM_FILE), shape, config.sys.beta, converged)
...
def _append_jacobian_eigenvalues(tracker: JacobianTracker, rows_lam: list, rows_res: list, rows_damping: list) -> None:
    ...
    ``jacobian_eigenvalues.npy``, ``jacobian_eigenvalue_residuals.npy`` and ``jacobian_damping.npy`` in the output
    folder.

    An iteration whose tracker update produced no estimate (see :attr:`JacobianTracker.estimated`) appends a row
    of ``nan`` to the first two, so the three files end up with exactly one row per iteration of this run, readable
    as the leading eigenvalue's approach to the instability and the damping the tracker ran at. The damping row
    always carries numbers, since the effective damping and the flipped-direction bound hold their last values
    through an iteration without an estimate.
    ...
    :param rows_damping: The matching ``(p_eff, p_flip)`` rows collected so far, oldest first, extended in place.
    :return: None.
    """
    ...
    rows_res.append(res)
    rows_damping.append((tracker.p_eff, tracker.p_flip))
    np.save(os.path.join(config.output.output_path, EIGENVALUE_FILE), np.array(rows_lam, dtype=np.complex128))
    np.save(os.path.join(config.output.output_path, EIGENVALUE_RESIDUAL_FILE), np.array(rows_res, dtype=np.float64))
    np.save(os.path.join(config.output.output_path, DAMPING_FILE), np.array(rows_damping, dtype=np.float64))
...
def calculate_self_energy_q(
    ...
    certified spectrum allows, which the mixing applies to every damped Picard step and to every iteration with a
    reflection in force; a quasi-Newton step without one keeps the configured parameter.
            ...
            if config.stabilization.use_chi_phys_restriction:
                ...
                if tracker is not None and comm.rank == 0:
                    logger.info(
                        ...
                        f"directions, p_eff={tracker.p_eff:.4f}, p_flip={tracker.p_flip:.4f}, "
                        f"rho={tracker.predicted_rate():.4f}."
                    )
                break
        else:
    ...
    if tracker is not None and comm.rank == 0:
        _save_jacobian_spectrum(tracker, pure_converged)
...
def apply_mixing_strategy(
    ...
    an accelerated step takes :attr:`JacobianTracker.p_flip`, the bound of the flipped directions alone, while a
    reflected basis is installed and keeps the configured parameter otherwise, since the whole-spectrum bound
    describes the damped iteration and not a quasi-Newton step.
    ...
    Whenever the tracker has a flipped basis :math:`Q` installed, the proposal is reflected on it,

    .. math:: \tilde{S}(x) = x + (\mathbb{1} - 2 Q Q^{T})\,(S(x) - x),

    before anything else touches it: the reflected proposal enters the core window of ``sigma_new`` and is what
    every mixing strategy below acts on, regardless of whether ``mixing_history`` is given. When it is, the raw
    proposal is what gets recorded, since the tracker estimates the spectrum from it, and the reflected one is what
    the accelerated schemes take as their map result.
    ...
    # a quasi-Newton step is damped by the flip bound only while a reflection is in force (see JacobianTracker)
    alpha_accelerated = tracker.p_flip if tracker_active else p_config
    ...
    if tracker_active or mixing_history is not None:
        ...
        if mixing_history is not None:
            ...
            n_keep = max(config.self_consistency.mixing_history_length, tracker.n_pairs if tracker is not None else 0)
    ...
    if config.self_consistency.mixing_strategy.lower() == "pulay" and accelerated_mixing_condition:
        ...
        update = alpha_accelerated * f_i - (r_matrix + alpha_accelerated * f_matrix) @ coeffs
        ...
        logger.info(f"Pulay mixing applied (m={n_hist}, alpha={alpha_accelerated:.3f}, norm_f={norm_f:.3e}).")
    ...
    if config.self_consistency.mixing_strategy.lower() == "anderson" and accelerated_mixing_condition:
        ...
        x_n_complex = x_n
        candidate = (1 - alpha_accelerated) * x_n_complex + alpha_accelerated * x_anderson.reshape(-1)
        ...
        logger.info(f"Anderson acceleration applied (m={n_hist}, alpha={alpha_accelerated:.3f}, norm_f={norm_f:.3e}).")
