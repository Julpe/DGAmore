...
class GreensFunction(TwoPoint):
    ...
    def get_fill_nonlocal_from_sigma(
        ...
        * the interacting part is built, inverted and frequency-summed in momentum chunks of at most
        ...
        * the model part :math:`G_{\mathrm{mod}} = [\imath\nu + \mu - \varepsilon_{\mathbf{k}} -
          ...
          \Sigma_\infty` (one eigendecomposition per k instead of a second inversion pass), in momentum chunks of
          the same budget.

        Both frequency sums run over the full fermionic axis for every momentum, so the summation order - and with
        it the last bit of the result - is independent of how many momenta ``ek`` carries: a momentum slice reports
        exactly the rows the full-grid evaluation reports.
        ...
        static_bands = mu_bands[None, :, :, None] - ek.reshape(nk_tot, n_bands, n_bands)[..., None]
        sigma_mat = siw.decompress_q_dimension().mat.reshape(-1, n_bands, n_bands, 2 * niv)
        iv_bands = (iv[None, None, :] * eye_bands[..., None])[None, ...]
        box_sum = np.empty((nk_tot, n_bands, n_bands))
        step = max(1, _MODEL_EPOT_CHUNK_ELEMENTS // (n_bands**2 * 2 * niv))
        for start in range(0, nk_tot, step):
            stop = min(nk_tot, start + step)
            dyson = iv_bands + static_bands[start:stop]
            # a momentum-local sigma holds a single momentum row and broadcasts over every chunk
            dyson -= sigma_mat if sigma_mat.shape[0] == 1 else sigma_mat[start:stop]
            box_sum[start:stop] = np.sum(GreensFunction._invert_last_orbital_block(dyson).real, axis=-1)

        # in-box sum of the model G: sum_v G_mod = U diag(sum_v 1/(iv + mu - lambda)) U^-1 with h_k = eps_k + Sigma_inf
        # = U diag(lambda) U^-1, evaluated in [k-chunk, band, v]-blocks; only the [k, o, o] result is rotated back
        h = (ek + smom0[None, None, None]).reshape(nk_tot, n_bands, n_bands)
        lam, u = np.linalg.eig(h)
        u_inv = np.linalg.inv(u)
        step = max(1, _MODEL_EPOT_CHUNK_ELEMENTS // (n_bands * 2 * niv))
        model_diag_sum = np.empty((nk_tot, n_bands), dtype=np.complex128)
        for start in range(0, nk_tot, step):
            stop = min(nk_tot, start + step)
            model_diag_sum[start:stop] = (1.0 / (iv[None, None, :] + mu - lam[start:stop, :, None])).sum(axis=-1)
        box_sum -= ((u * model_diag_sum[:, None, :]) @ u_inv).real
        box_sum = box_sum.reshape(*nk, n_bands, n_bands)
