...
def test_install_exact_spectrum_is_the_saved_snapshot_and_carries_into_a_successor(tmp_path):
    ...
    state = tracker.spectrum_state(shape, 10.0, True, np.complex128)
    ...
    path = str(tmp_path / JACOBIAN_FILE)
    tracker.save_spectrum(path, shape, 10.0, True, np.complex128)
    loaded = load_spectrum(path)
    assert bool(loaded["exact"])
    successor = JacobianTracker(0.4)
    assert successor.carry_in(loaded, lambda column: to_vec(np.asarray(column).reshape(shape)))
    assert successor.q.shape[1] == 1 and np.allclose(np.abs(successor.q[:, 0]), [1.0, 0.0, 0.0, 0.0], atol=1e-12)
    assert not bool(successor.spectrum_state(shape, 12.0, True, np.complex128)["exact"])


def test_a_later_certifying_update_replaces_the_exact_snapshot():
    """The exact snapshot stands until the tracker's own estimate certifies a mode again."""
    tracker = JacobianTracker(0.4)
    tracker.install_exact_spectrum(*_exact_modes())
    tracker.update(*linear_pairs(np.diag([0.4, 1.5]), np.zeros(2), 0.4, TRACKER_PAIRS, [1.0, 1.0]))

    state = tracker.spectrum_state((1, 1, 1, 1, 1, 2), 10.0, True, np.complex128)
    assert tracker.last_gated.any() and not bool(state["exact"])
...
def _band_mode_tracker() -> JacobianTracker:
    ...
    tracker = JacobianTracker(0.4)
    iterates, proposals = linear_pairs(np.diag([0.4, 1.005, 0.7]), np.zeros(3), 0.4, 9, [1.0, 1.0, 1.0])
    for end in range(3, 10):
        tracker.update(iterates[:end], proposals[:end])
    return tracker
...
def test_a_band_mode_close_to_a_flat_one_keeps_the_whole_certified_span_mapped():
    ...
    gains[1, 2] = 0.2
    iterates, proposals = linear_pairs(gains, np.zeros(3), 0.4, 9, [1.0, 1.0, 1.0])
    for end in range(3, 10):
        tracker.update(iterates[:end], proposals[:end])
...
def test_the_floor_warning_does_not_call_the_branch_unreachable():
    """The floor warning names the damping bound and the per-direction map instead of an unreachable branch."""
    logger = MagicMock()
    tracker = JacobianTracker(0.4, logger=logger)
    iterates, proposals = linear_pairs(np.diag([201.0, 0.5]), np.array([1.0, 1.0]), 0.1, 9, [0.05, 0.05])
    for end in range(3, len(iterates) + 1):
    ...
    assert "floor" in message and "not reachable" not in message and "every damped step runs at" in message
