...
def test_find_lambda_bounded_without_a_sign_change_and_no_previous_keeps_zero_inside_the_branch():
    """An unreachable target with no previous lambda on a branch containing zero falls back to zero."""
    logger = _sum_rule_config()
    chi = _healthy_chi_qw()
    ...
    assert "zero (no correction)" in logger.warning.call_args_list[0].args[0]


def test_find_lambda_bounded_without_a_sign_change_and_no_previous_keeps_lower_end_plus_delta_above_zero():
...
def test_find_lambda_bounded_without_a_sign_change_keeps_a_genuine_previous_lambda_and_warns():
    """A genuine previous lambda inside the branch is used ahead of the neutral value when no root exists."""
    logger = _sum_rule_config()
    chi = _healthy_chi_qw()
    previous = 0.5
    ...
    assert result == previous and is_fallback
...
def test_perform_routes_the_per_iteration_path_to_the_bounded_finder(monkeypatch):
    ...
    _spch_perform_config()
    chi = load_four_point("spch", "chi_phys_q_magn_before_lambda", SpinChannel.MAGN)
    plain, bounded = MagicMock(return_value=1.0), MagicMock(return_value=(2.0, False))
    lambda_previous = {}
