...
def _fq_transient(chunk):
    """The modeled pairing-vertex chunk transient: the window, its matmul copies and the sliced local vertex."""
...
def test_sde_single_adds_the_jacobian_tracker_term():
    """The tracker flag adds its rank-0 history, three resident Ritz sets, four tall arrays and its transient to sde."""
    ...
    assert extra == pytest.approx(history + ritz_vectors + tall + transient)
