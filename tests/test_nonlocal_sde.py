...
def test_loop_tracker_runs_on_rank0_only_on_two_node_ranks(monkeypatch, tmp_path):
    """On two ranks of one node only rank 0 updates the tracker and saves the proposal; both ranks return one sigma."""
    monkeypatch.setattr(mpi_utils, "MPI", FAKE_MPI)
    monkeypatch.setattr(nonlocal_sde, "MPI", FAKE_MPI)
    run, calls, _ = _setup_self_energy_loop(
        monkeypatch, tmp_path, _affine_core_step(_STABLE_GAINS), max_iter=4, epsilon=0.0
    )
    config.stabilization.use_jacobian_stabilization = True
    config.sys.n_bands = 1
    ...
    # rank 0's own sigma_dga save decompresses its momentum axis, so the ranks are compared on values, not on layout
    assert len(calls) == 8 and updates == ["rank0"] * 4 and saves == ["rank0"] * 4
    assert any(estimates)
    assert np.array_equal(results[0].mat.reshape(-1), results[1].mat.reshape(-1))
...
def test_save_jacobian_spectrum_passes_the_window_shape_and_the_path(tmp_path):
    ...
    config.sys.n_bands = 1
    tracker = MagicMock()
    rows = ([np.ones(3, dtype=np.complex128)], [np.zeros(3)], [0.4])
    ...
    assert kwargs["traces"]["eigenvalues"].shape == (1, 3) and kwargs["traces"]["eigenvalue_residuals"].shape == (1, 3)
